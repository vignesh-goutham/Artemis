...
class Backtester:
    def __init__(self, initial_balance: float = 10000):
        ...
        self.cash_history = []  # Track cash balance over time
        self.active_positions = {}  # {ticker: {'shares': float, 'basis_price': float}}
    ...
    def run_backtest(self) -> None:
        # Sort trades by buy date to process them in chronological order
        self.trades.sort(key=lambda x: x.buy_date)
        ...
        for trade in self.trades:
            try:
                ...
                allocation_amount = self.current_balance * (trade.allocation_percentage / 100)
                
                # Check if we have enough cash
                if allocation_amount > self.current_balance:
                    print(f"Warning: Insufficient cash for {trade.ticker}. Required: ${allocation_amount:,.2f}, Available: ${self.current_balance:,.2f}")
                    continue
                ...
                # Execute the trade
                trade.execute(allocation_amount)
                
                # Update cash balance
                self.current_balance -= allocation_amount
                
                # Add to active positions
                self.active_positions[trade.ticker] = {
                    'shares': trade.initial_shares,
                    'basis_price': trade.buy_price
                }
                
                # Record cash balance after initial trade
                self.cash_history.append({
                    'date': trade.buy_date,
                    'balance': self.current_balance,
                    'type': 'post_initial_trade'
                })
                
                # Process partial sells and update cash balance
                if hasattr(trade, 'partial_sells') and trade.partial_sells:
                    for partial_sell in trade.partial_sells:
                        # Add profit/loss to cash balance
                        self.current_balance += partial_sell['profit_at_sell']
                        
                        # Update active position
                        self.active_positions[trade.ticker]['shares'] -= partial_sell['shares']
                        
                        # Record cash balance after partial sell
                        self.cash_history.append({
                            'date': partial_sell['date'],
                            'balance': self.current_balance,
                            'type': 'partial_sell'
                        })
                
                # Add final profit/loss to cash balance
                self.current_balance += trade.profit_loss
                
                # Remove from active positions if fully sold
                if trade.total_shares == 0:
                    del self.active_positions[trade.ticker]
                
                # Record cash balance after final sell
                self.cash_history.append({
                ...
                # Only add to results if we actually made the trade
        ...
        # Calculate final portfolio value including remaining positions
        ...
        if self.active_positions:
            ...
            for ticker, position in self.active_positions.items():
                ...
                self.final_portfolio_value += position_value
                
                print(f"{ticker}:")
                print(f"  Shares: {position['shares']:,.4f}")
                print(f"  Basis Price: ${position['basis_price']:,.2f}")
                print(f"  Current Price: ${current_price:,.2f}")
                print(f"  Position Value: ${position_value:,.2f}")
                print(f"  Unrealized P/L: ${(current_price - position['basis_price']) * position['shares']:,.2f}")
                print(f"  Unrealized P/L %: {((current_price - position['basis_price']) / position['basis_price'] * 100):,.2f}%")
    ...
    def print_results(self, detailed: bool = False) -> None:
        if detailed:
            ...
            for result in self.results:
                ...
                if hasattr(trade, 'partial_sells') and trade.partial_sells:
                    # Calculate total position before any partial sells
                    total_initial_shares = result['initial_shares']
                    total_initial_investment = result['initial_investment']
                    
                    # Add shares and investment from dip buys
                    for dip_buy in trade.dip_buys:
                        total_initial_shares += dip_buy['shares']
                        total_initial_investment += dip_buy['investment']
                    
                    # Add shares and investment from momentum buys
                    for momentum_buy in trade.momentum_buys:
                        total_initial_shares += momentum_buy['shares']
                        total_initial_investment += momentum_buy['investment']
                    
                    # Calculate what the total position would be worth at end date
                    total_end_value = (result['sell_price'] - result['buy_price']) * total_initial_shares
                    total_end_pct = ((result['sell_price'] - result['buy_price']) / result['buy_price']) * 100
                    
                    # Track remaining shares after each partial sell
                    remaining_shares = total_initial_shares
                    remaining_investment = total_initial_investment
                    total_partial_sell_profit = 0
                    ...
                    for partial_sell in trade.partial_sells:
                        ...
                        remaining_shares -= partial_sell['shares']
                        remaining_investment -= (partial_sell['shares'] * partial_sell['basis_price'])
                        total_partial_sell_profit += actual_profit
                        ...
                        partial_sell_trade = {
                            ...
                            'Remaining Shares': f"{remaining_shares:,.4f}",
                            'Remaining Investment': f"${remaining_investment:,.2f}",
                            'Dip Buy': 'No',
                            'Momentum Buy': 'No',
                    ...
                    final_position_value = remaining_shares * result['sell_price']
                    total_value = total_partial_sell_profit + final_position_value
                    total_return_pct = ((total_value - total_initial_investment) / total_initial_investment) * 100
                    
                    # Add summary row for this trade
                    summary_trade = {
                        'Ticker': result['ticker'],
                        'Buy Date': result['buy_date'],
                        'Sell Date': result['sell_date'],
                        'Buy Price': f"${result['buy_price']:,.2f}",
                        'Sell Price': f"${result['sell_price']:,.2f}",
                        'Shares': f"{total_initial_shares:,.4f}",
                        'Investment': f"${total_initial_investment:,.2f}",
                        'Partial Sell Profit': f"${total_partial_sell_profit:,.2f}",
                        'Final Position Value': f"${final_position_value:,.2f}",
                        'Total Value': f"${total_value:,.2f}",
                        'Total Return %': f"{total_return_pct:,.2f}%",
                        'Hold Until End Value': f"${total_end_value:,.2f}",
                        'Hold Until End Return %': f"{total_end_pct:,.2f}%",
                        'Dip Buy': 'No',
                        'Momentum Buy': 'No',
                        'Partial Sell': 'Summary'
                    }
            ...
            trade_summaries = trades_df[trades_df['Partial Sell'] == 'Summary']
            
            if not partial_sells.empty:
                print("\n=== Partial Sell Analysis ===")
                
                # Calculate totals from trade summaries
                total_initial_investment = sum(float(x.strip('$').replace(',', '')) for x in trade_summaries['Investment'])
                total_partial_sell_profit = sum(float(x.strip('$').replace(',', '')) for x in trade_summaries['Partial Sell Profit'])
                total_final_position = sum(float(x.strip('$').replace(',', '')) for x in trade_summaries['Final Position Value'])
                total_hold_value = sum(float(x.strip('$').replace(',', '')) for x in trade_summaries['Hold Until End Value'])
                
                print("Overall Performance:")
                print(f"1. Total Initial Investment: ${total_initial_investment:,.2f}")
                print(f"2. Total Profit from Partial Sells: ${total_partial_sell_profit:,.2f}")
                print(f"3. Total Final Position Value: ${total_final_position:,.2f}")
                print(f"4. Total Value (Partial Sells + Final Position): ${(total_partial_sell_profit + total_final_position):,.2f}")
                print(f"5. Total Value if Held Until End: ${total_hold_value:,.2f}")
                
                # Calculate return percentages
                actual_return_pct = ((total_partial_sell_profit + total_final_position - total_initial_investment) / total_initial_investment) * 100
                hold_return_pct = ((total_hold_value - total_initial_investment) / total_initial_investment) * 100
                return_difference_pct = actual_return_pct - hold_return_pct
                
                print(f"\nReturn Analysis:")
                print(f"1. Actual Return: {actual_return_pct:,.2f}%")
                print(f"2. Hold Until End Return: {hold_return_pct:,.2f}%")
                if return_difference_pct > 0:
                    print(f"3. Additional Return from Partial Sells: {return_difference_pct:,.2f}%")
                else:
                    print(f"3. Return Lost by Partial Sells: {abs(return_difference_pct):,.2f}%")
                ...
                for reason in ['take_profit', 'cut_loss', 'overbought', 'volatility']:
                    ...
                    if not reason_sells.empty:
                        ...
                        print(f"  Total Profit: ${reason_profit:,.2f}")
                        print(f"  Return: {reason_return_pct:,.2f}%")
            ...
            # Add cash balance analysis
            print("\n=== Cash Balance Analysis ===")
            cash_df = pd.DataFrame(self.cash_history)
            cash_df['date'] = pd.to_datetime(cash_df['date'])
            cash_df = cash_df.sort_values('date')
            
            print("\nCash Balance Timeline:")
            for _, row in cash_df.iterrows():
                print(f"{row['date'].strftime('%Y-%m-%d')} - {row['type']}: ${row['balance']:,.2f}")
            
            # Calculate cash utilization
            total_allocated = sum(result['total_investment'] for result in self.results)
            avg_cash_utilization = (total_allocated / self.initial_balance) * 100
            
            print(f"\nCash Utilization:")
            print(f"Total Allocated: ${total_allocated:,.2f}")
            print(f"Average Cash Utilization: {avg_cash_utilization:,.2f}%")
            
            # Print final portfolio value
            print("\n=== Final Portfolio Value ===")
            print(f"Cash Balance: ${self.current_balance:,.2f}")
            print(f"Remaining Positions Value: ${(self.final_portfolio_value - self.current_balance):,.2f}")
            print(f"Total Portfolio Value: ${self.final_portfolio_value:,.2f}")
            print(f"Total Return: ${(self.final_portfolio_value - self.initial_balance):,.2f}")
            print(f"Total Return %: {((self.final_portfolio_value - self.initial_balance) / self.initial_balance * 100):,.2f}%")
        ...
        # Calculate returns
        total_return = self.final_portfolio_value - self.initial_balance
        ...
        print(f"Final Balance: ${self.final_portfolio_value:,.2f}")
